...
export * from '@/lib/utils/time-slots';
...
export function mergeTimeRanges(ranges: TimeRange[]): TimeRange[] {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (doTimeRangesOverlap(last, current) || timeToMinutes(last.end) === timeToMinutes(current.start)) {
...
export function generateTimeRange(startTime: string, endTime: string, intervalMinutes: number): string[] {
  const times: string[] = [];
  let currentTime = startTime;
  
  while (isTimeBefore(currentTime, endTime) || isTimeEqual(currentTime, endTime)) {
...
export function getTimeRangeDuration(startTime: string, endTime: string): string {
...
export function getNextAvailableTime(currentTime: string, intervalMinutes: number): string {
  return addMinutesToTime(currentTime, intervalMinutes);
}

export function getPreviousAvailableTime(currentTime: string, intervalMinutes: number): string {
