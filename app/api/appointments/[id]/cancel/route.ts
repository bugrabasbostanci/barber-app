...
export async function POST(
  ...
  try {
    ...
    const appointmentDateTime = new Date(`${appointment.date.toISOString().split('T')[0]}T${appointment.startTime.toTimeString().substring(0, 8)}`)
    const now = new Date()
    const hoursDiff = (appointmentDateTime.getTime() - now.getTime()) / (1000 * 60 * 60)

    if (hoursDiff < 2) { // BUSINESS_RULES.CANCELLATION_HOURS = 2
      return NextResponse.json({ 
