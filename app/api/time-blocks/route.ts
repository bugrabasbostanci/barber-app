...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedBlocks = timeBlocks.map(block => ({
      ...
      startTime: block.startTime ? block.startTime.toTimeString().substring(0, 5) : null,
      endTime: block.endTime ? block.endTime.toTimeString().substring(0, 5) : null,
      reason: block.reason,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { date, staffId, startTime, endTime, reason, isFullDay } = await request.json()
    ...
    return NextResponse.json({ 
      ...
      timeBlock: {
        ...
        startTime: timeBlock.startTime ? timeBlock.startTime.toTimeString().substring(0, 5) : null,
        endTime: timeBlock.endTime ? timeBlock.endTime.toTimeString().substring(0, 5) : null,
