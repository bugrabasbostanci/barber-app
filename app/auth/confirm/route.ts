  ...
  if (token_hash && type) {
    ...
    if (!error) {
      ...
      if (user && type === 'signup') {
        ...
        if (result.success) {
          return NextResponse.redirect(new URL('/auth/login?message=Hesabınız başarıyla oluşturuldu! Giriş yapabilirsiniz.', request.url))
        } else {
          console.error('Failed to create user in database:', result.error)
          return NextResponse.redirect(new URL('/auth/login?message=E-posta doğrulandı ancak hesap oluşturmada sorun yaşandı. Lütfen giriş yapmayı deneyin.', request.url))
        }
      }
