...
import { BookingData, CustomerInfo, Staff } from "../types/booking.types";
...
export const formatPhoneInput = (value: string): string => {
  ...
  if (cleaned.startsWith("+90")) {
    ...
    if (digits.length <= 10) {
      return (
        "+90 " +
        digits.replace(/(\d{3})(\d{3})(\d{2})(\d{2})/, "$1 $2 $3 $4").trim()
      );
    }
    return (
      "+90 " +
      ...
      submitBooking: async () => {
        ...
        try {
          ...
          if (
            ...
            state.customerInfo.phone !== state.userProfile.phone
          ) {
          ...
          const payload = {
            ...
            // Notes would be handled separately from customer info
          };
          ...
          if (response.ok && result.success) {
            ...
            try {
              const { useAppointmentsStore } = await import(
                "../../appointments/stores/appointmentsStore"
              );
              useAppointmentsStore.getState().invalidateCache();
            } catch {
              // Appointments store not available
              console.log(
                "Appointments store not available during booking submission"
              );
            }
